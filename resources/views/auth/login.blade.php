...
@section('content')
    ...
    <div class="relative flex items-center justify-center w-full min-h-screen px-4 py-8 sm:px-6">
        ...
        <div class="relative p-6 overflow-hidden shadow-md w-full max-w-md shadow-yellow-400/40 backdrop-blur-xl bg-white/5 rounded-2xl sm:p-8 md:p-10">
            ...
            @if (session('status'))
                <p class="mt-1 text-sm text-red-400 text-center">{{ session('status') }}</p>
            @endif
            <!-- Laravel Form -->
            <form method="POST" action="{{ route('auth.loginpost') }}" class="space-y-6">
                ...
                <!-- Login Button -->
                <button type="submit" class="cursor-pointer relative w-full hover:opacity-90 hover:bg-gradient-to-l transition-all py-3 overflow-hidden rounded-xl bg-gradient-to-r from-yellow-500 to-lime-500">
                    Login
                </button>
            </form>
            ...
            <div class="mt-8 text-center">
                <p class="text-gray-400">
                    Don't have an account?
                     <a href="{{ route('auth.signup') }}" class="font-medium transition-colors text-lime-400 hover:text-yellow-400">
                    Sign up
                    </a>
                </p>
            </div>
